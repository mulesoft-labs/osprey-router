...
Engine.prototype.handle = function handle (req, res, callback) {
  ...
  function next (err) {
    ...
    self.processParams(match, paramcalled, req, res, function (err) {
      if (err) {
...
Engine.prototype.processParams = function processParams (match, called, req, res, done) {
...
Engine.prototype.use = function use (path, match, handlers) {
  ...
  var layerMatch = path === '/' ? fastSlash : match
